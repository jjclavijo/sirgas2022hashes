...
#include "../include/t_rinex.h"

void dumpobs(obs_t *obs)
{
    gtime_t time={0};
    int i;
    char str[64];
    ...
    for (i=0;i<obs->n;i++) {
        ...
        /*assert(1<=obs->data[i].sat&&obs->data[i].sat<=32);*/ /*Hay satelites > 32 */
...
static rnxopt_t opt2= {
    {0},{0},0.0,0.0,1.0,2.10,SYS_ALL,OBSTYPE_ALL,FREQTYPE_ALL,{{0}},
    "STAID",
...
void epoch2time_(gtime_t *time, const double *ep)
{ *time = epoch2time(ep);
}

extern int readrnxt_(const char *file, int rcv, gtime_t *ts, gtime_t *te, double tint, const char *opt, obs_t *obs, nav_t *nav, sta_t *sta)
{
  return readrnxt(file, rcv, *ts, *te, tint, opt, obs, nav, sta);
}
